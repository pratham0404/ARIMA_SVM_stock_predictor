...
class CSO:
    ...
    def __init__(self, fitness, P=150, n=2, pa=0.25, beta=1.5, bound=None, 
                plot=False, min=True, verbose=False, Tmax=300,discrete=0):
        ...
        dicrete: NUMBER OF DISCRETE PARAMETERS
        ...
        for i in range(self.P):
            self.X[i]=self.discretize(self.X[i])
            ...
            for j in range(self.n):
                ...
                if r < self.pa:
                    ...
                    if self.bound is not None:
                        ...
                        Xnew[i,j]=np.clip(Xnew[i,j],xmin,xmax)
                    if j<self.discrete: Xnew[i,j]=round(Xnew[i,j])
            self.X[i,:] = self.optimum(Xnew[i,:], self.X[i,:])
    ...
    def clip_Xnew(self,Xnew):

        # IF BOUND IS SPECIFIED THEN CLIP 'X new' VALUES SO THAT THEY ARE IN THE SPECIFIED RANGE
